    ...
    >
      ...
      {activeTab === "sessions" ? (
        <div className="mt-6 space-y-6">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <MetricCard label="Total Users" value={adminAnalytics.total_users || 0} />
                  ...
                  <div className="space-y-4">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                      ...
                      <div className="mt-1 text-sm text-slate-400">
                        {adminHistory.role} • {adminHistory.conversation_count} chats • {" "}
                        {adminHistory.message_count} messages
                      </div>
                    </div>
      ...
      {isSuperAdmin && activeTab === "content" ? (
        <div className="mt-6 grid gap-4">
          {Object.entries(contentDrafts).map(([key, value]) => (
            <div
              className="rounded-[1.5rem] border border-white/10 bg-slate-950/70 p-5"
              key={key}
            >
